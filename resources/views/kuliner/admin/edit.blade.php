...
@section('title', 'Edit Warung - Admin Panel')
...
.kf-page-title i { color:#D10024; font-size:20px; }
...
.kf-section-divider i { color:#D10024; }
...
.kf-input:focus, .kf-select:focus, .kf-textarea:focus {
    border-color:#D10024; background:#fff; box-shadow:0 0 0 3px rgba(209,0,36,.1);
}
...
.kf-time-sep { font-size:16px; font-weight:800; color:#D10024; }

.kf-current-photo { border-radius:10px; overflow:hidden; margin-bottom:10px; }
.kf-current-photo img { width:100%; max-height:220px; object-fit:cover; display:block; }
.kf-current-photo-label { padding:8px 13px; background:#f8f9fb; border:1px solid #f0f0f0; border-top:none; border-radius:0 0 10px 10px; font-size:11px; color:#8d8d8d; display:flex; align-items:center; gap:6px; }
.kf-no-photo { height:100px; background:linear-gradient(135deg,#fee2e2,#fecaca); border-radius:10px; display:flex; align-items:center; justify-content:center; gap:8px; margin-bottom:10px; font-size:12px; color:#991b1b; }
.kf-no-photo i { font-size:22px; opacity:.5; }
...
.kf-upload-placeholder i { font-size:24px; color:#D10024; opacity:.5; }
...
.kf-footer { display:flex; align-items:center; justify-content:space-between; gap:10px; padding:18px 24px; border-top:1px solid #f2f2f5; background:#f8f9fb; }
.kf-footer-right { display:flex; align-items:center; gap:10px; }
...
.kf-delete { padding:10px 16px; background:#fff; color:#991b1b; border:1.5px solid #fecaca; border-radius:9px; font-size:13px; font-weight:700; cursor:pointer; font-family:inherit; transition:all .15s; display:inline-flex; align-items:center; gap:6px; }
.kf-delete:hover { background:#fee2e2; border-color:#fca5a5; }
</style>

<div class="kf-wrap">
    <div class="admin-hero">
        <div class="admin-hero-left">
            <div class="admin-hero-icon"><i class="fa fa-pencil"></i></div>
            <div>
                <h1 class="admin-hero-title">Edit Warung: {{ $kuliner->nama }}</h1>
                <p class="admin-hero-sub">Perbarui informasi warung kuliner di NusaMart</p>
            </div>
        </div>
    ...
    <form action="{{ route('admin.kuliner.update', $kuliner->id) }}" method="POST" enctype="multipart/form-data" id="editForm">
    @csrf @method('PUT')
    <div class="kf-card">
        ...
        <div class="kf-body">
            <div class="kf-grid2">
                <div class="kf-group full">
                    ...
                    <input type="text" name="nama" class="kf-input" value="{{ old('nama', $kuliner->nama) }}">
        ...
        <div class="kf-body">
            @if($kuliner->gambar && file_exists(public_path('uploads/'.$kuliner->gambar)))
                <div class="kf-current-photo">
                    <img src="{{ asset('uploads/'.$kuliner->gambar) }}" alt="{{ $kuliner->nama }}">
                    <div class="kf-current-photo-label"><i class="fa fa-image"></i> Foto saat ini — pilih file baru di bawah untuk menggantinya</div>
                </div>
            @else
                <div class="kf-no-photo"><i class="fa fa-image"></i> Belum ada foto</div>
            @endif
        ...
        <div class="kf-body">
            <div class="kf-grid2">
                <div class="kf-group full">
                    ...
                    <input type="text" name="alamat" class="kf-input" value="{{ old('alamat', $kuliner->alamat) }}">
                ...
                <div class="kf-group">
                    ...
                    <div class="kf-time-row">
                        <input type="time" name="jam_buka"  class="kf-input" value="{{ old('jam_buka',  $kuliner->jam_buka) }}"  style="flex:1">
                        <span class="kf-time-sep">–</span>
                        <input type="time" name="jam_tutup" class="kf-input" value="{{ old('jam_tutup', $kuliner->jam_tutup) }}" style="flex:1">
                    </div>
                ...
                <div class="kf-group">
                    ...
                    <input type="text" name="kontak_wa" class="kf-input" value="{{ old('kontak_wa', $kuliner->kontak_wa) }}">
        ...
        <div class="kf-footer">
            ...
            <form action="{{ route('admin.kuliner.destroy', $kuliner->id) }}" method="POST"
                ...
                @csrf @method('DELETE')
                <button type="submit" class="kf-delete"><i class="fa fa-trash"></i> Hapus Warung</button>
            </form>
            <div class="kf-footer-right">
                <a href="{{ route('admin.kuliner.index') }}" class="kf-cancel"><i class="fa fa-times"></i> Batal</a>
                <button type="submit" form="editForm" class="kf-submit"><i class="fa fa-save"></i> Simpan Perubahan</button>
            </div>
        </div>
    </div>
    </form>
