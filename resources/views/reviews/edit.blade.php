...
<div class="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-2xl mx-auto bg-white rounded-lg shadow-md p-6">
        <!-- Header -->
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-900 mb-2">Ubah Ulasan</h1>
            <p class="text-gray-600">Perbarui ulasan Anda untuk produk ini</p>
        </div>

        <!-- Produk Info -->
        <div class="flex gap-4 mb-8 pb-8 border-b">
            @if($review->product->image)
                <img src="{{ asset('uploads/' . $review->product->image) }}" 
            ...
            <div class="flex-1">
                <h2 class="text-xl font-semibold text-gray-900">{{ $review->product->name }}</h2>
                <p class="text-gray-600 text-sm mt-1">{{ Str::limit($review->product->description, 100) }}</p>
